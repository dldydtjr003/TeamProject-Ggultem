...
public interface BoardRepository extends JpaRepository<Board, Integer> {
	...
	// 검색
	@EntityGraph(attributePaths = { "boardImage" })
	@Query("SELECT b FROM Board b WHERE b.enabled = 1 AND (" + "(:searchType = 'title' AND b.title LIKE %:keyword%) OR "
	...
	// =========================
	@EntityGraph(attributePaths = { "boardImage" })
	@Query("SELECT b FROM Board b WHERE " + "(:enabled IS NULL OR b.enabled = :enabled) AND "
			+ "(:keyword IS NULL OR b.title LIKE %:keyword% OR b.content LIKE %:keyword%)")
	Page<Board> searchAllAdmin(@Param("enabled") Integer enabled, @Param("keyword") String keyword, Pageable pageable);
}